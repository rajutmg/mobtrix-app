...
const AddProject = () => {
  ...
  const managerurl = `${localStorage.api}/managers`;
  ...
  const getClient = async () => {
    ...
    let res1 = await axios.get(managerurl, headers);
    setManager(res1.data.data);
  };

  const editProject = async (e) => {
    e.preventDefault();
    let res = await axios.put(url, project, headers).catch((error) => {
  ...
  return (
    <div className="mt-3">
      ...
      <form noValidate autoComplete="off" onSubmit={editProject}>
